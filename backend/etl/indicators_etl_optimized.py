...
def fetch_price_df_sync(symbol: str, start: Optional[datetime]) -> pd.DataFrame:
    ...
    return df[df["close"].notna()][["open","high","low","close","volume"]]
...
def bulk_upsert(symbol: str, ind: pd.DataFrame):
    ...
    if ops:
        try:
            ...
            print(f"Bulk write error for {symbol}: {e.details}")
...
def run_indicators_etl_sync(incremental: bool = True):
    ...
    print(f"Running indicators ETL for {len(symbols)} symbols (incremental={incremental})")
